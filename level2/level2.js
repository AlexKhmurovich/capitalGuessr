...
startBtn.onclick = function () {
   ...
   attemptsText.innerHTML = `Attempts: ${attempts}/10`;
   collectList(2);
   getRandomCountry();
   setHeading();
};
...
guessBtn.onclick = function () {
   ...
   if (userAnswerCap == randCapital) {
      score++;
      attempts++;
      attemptsText.innerHTML = `Attempt: ${attempts}/10`;
      scoreText.innerHTML = `Score: ${score}`;
      getRandomCountry();
      setHeading();
      textBox.value = "";
      guessBtn.innerHTML = "Guess";
   } else if (userAnswerCap != randCapital && attempts == 9) {
      attempts++;
      attemptsText.innerHTML = `Attempt: ${attempts}/10`;
      scoreText.innerHTML = `Score: ${score}`;
      maxAttemptsReached();
...
function setHeading() {
   ...
   heading.style.setProperty("filter", "drop-shadow(0 0 20px yellow)");
   textBox.style.display = "inline";
   guessBtn.style.display = "inline";
}
